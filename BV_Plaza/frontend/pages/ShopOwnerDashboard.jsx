...
const ProductsPanel = ({ products, setProducts, toast }) => {
  ...
  const save = async () => {
    ...
    try {
      if (editId) {
        const r = await apiPlazaUpdateProduct(editId, { ...form, price: Number(form.price), stock: Number(form.stock||0), discount: Number(form.discount||0) });
        setProducts(prev => prev.map(p => p._id === editId ? r.data.product : p));
        toast?.("Product updated!");
      } else {
        const r = await apiPlazaCreateProduct({ ...form, price: Number(form.price), stock: Number(form.stock||0), discount: Number(form.discount||0) });
    ...
    <div>
      ...
      <div style={{ background:"#fff", borderRadius:16, border:"1px solid #e2e8f0", padding:24, marginBottom:28 }}>
        ...
        <div style={{ display:"flex", gap:10, marginTop:16 }}>
          ...
          {editId && <button onClick={()=>{setEditId(null);setForm({name:"",description:"",price:"",originalPrice:"",category:"General",stock:"",discount:"",videoUrl:""});}} style={{ padding:"11px 20px", background:"#f1f5f9", border:"none", borderRadius:12, fontWeight:600, cursor:"pointer" }}>Cancel</button>}
        </div>
      </div>
