...
import {
  ...
  apiAdminStats, apiAdminUsers, apiAdminCreateUser, apiAdminToggleUser, apiAdminChangeRole, apiAdminDeleteUser, apiUpdateProfile,
} from "./api";
      ...
      <header style={{
        ...
        <div style={{maxWidth:1440,margin:"0 auto",padding:isMobile?"0 16px":"0 48px",display:"flex",alignItems:"center",height:isMobile?62:74,gap:16}}>
          ...
          <div onClick={()=>go("home")} style={{cursor:"pointer",flex:isMobile?1:"none",display:"flex",alignItems:"center",justifyContent:isMobile?"center":"flex-start",gap:10}}>
            <img src="/bv.jpg" alt="BanyanVision" style={{width:isMobile?42:50,height:isMobile?42:50,objectFit:"cover",borderRadius:10,boxShadow:"0 2px 12px rgba(194,24,91,.25)"}}/>
            {!isMobile&&(
              <div>
                ...
                <div style={{fontSize:8,fontWeight:700,letterSpacing:3,color:"var(--muted)",textTransform:"uppercase",marginTop:2}}>Fashion & Lifestyle</div>
              </div>
            )}
          </div>
...
const CheckoutPage = ({ setPage, toast }) => {
  ...
  const pay=async()=>{
    ...
    try{
      ...
      const opts={
        key:keyId,amount,currency,name:"BanyanVision Fashion",order_id:orderId,
        handler:async res=>{
          try{
...
<style>
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .logo-box{width:36px;height:36px;background:linear-gradient(135deg,#C2185B,#E65100);border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;}
...
<div class="label">
  ...
  <div class="section">
    ...
    <div class="val" style="font-weight:700">BanyanVision Fashion</div>
  ...
  <div class="footer">
    Packed on: ${new Date(ord.createdAt).toLocaleDateString("en-IN")} · Payment: ${ord.paymentId?"Paid (Razorpay)":"COD"} · Thank you for shopping with BanyanVision!
  </div>
</div>
                  ...
                  <div>
                    ...
                    {stepIdx===3&&(
                      <div style={{marginTop:14,padding:"10px 14px",background:"#F0FDF4",borderRadius:10,fontSize:12,color:"#16A34A",fontWeight:600,display:"flex",alignItems:"center",gap:8}}>
                        <span>✅</span><span>Your order has been delivered. Thank you for shopping with BanyanVision!</span>
                      </div>
                    )}
                  </div>
                )}
      ...
      {!isMobile&&(
        <div style={{background:"linear-gradient(160deg,var(--rose) 0%,var(--saffron) 50%,var(--purple) 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:60,position:"relative",overflow:"hidden"}}>
          ...
          <div style={{position:"relative",textAlign:"center",maxWidth:380}}>
            <img src="/bv.jpg" alt="BanyanVision" style={{width:80,height:80,objectFit:"cover",borderRadius:16,margin:"0 auto 20px",boxShadow:"0 8px 32px rgba(0,0,0,.3)"}}/>
            <div style={{fontFamily:"var(--font-d)",fontSize:40,fontWeight:700,color:"#fff",marginBottom:6,letterSpacing:.5}}>BanyanVision</div>
            <div style={{fontSize:11,color:"rgba(255,255,255,.75)",letterSpacing:3,fontWeight:700,textTransform:"uppercase",marginBottom:32}}>Fashion & Lifestyle</div>
        ...
        <div style={{width:"100%",maxWidth:400}}>
          {isMobile&&(
            <div style={{textAlign:"center",marginBottom:28}}>
              <img src="/bv.jpg" alt="BanyanVision" style={{width:64,height:64,objectFit:"cover",borderRadius:14,margin:"0 auto 12px",boxShadow:"0 4px 20px rgba(194,24,91,.25)"}}/>
...
const AdminDashboard = ({ setPage, toast }) => {
  ...
  const SIDE=[["overview","◈","Overview"],["orders","◻","Orders"],["products","◈","Products"],["add-product","＋","Add Product"],["customers","◉","Customers"],["analytics","◈","Analytics"],["coupons","◈","Coupons"],["categories","🏷","Categories"],["promo","🎁","Offer Banner"],["about-editor","📄","About Page"]];
    ...
    <footer style={{background:"var(--dark)",borderTop:"2px solid var(--dark2)"}}>
      <div style={{maxWidth:1440,margin:"0 auto",padding:isMobile?"44px 24px 24px":"64px 80px 28px"}}>
        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"2fr 1fr 1fr 1fr",gap:isMobile?28:48,marginBottom:44}}>
          <div style={{gridColumn:isMobile?"1/-1":"auto"}}>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
              <img src="/bv.jpg" alt="BanyanVision" style={{width:44,height:44,objectFit:"cover",borderRadius:10,boxShadow:"0 4px 16px rgba(194,24,91,.3)"}}/>
              <div>
                <div style={{fontFamily:"var(--font-d)",fontSize:22,fontWeight:700,color:"#fff",letterSpacing:.5}}>BanyanVision</div>
                <div style={{fontSize:8,fontWeight:700,letterSpacing:3,color:"rgba(255,255,255,.4)",textTransform:"uppercase"}}>Fashion & Lifestyle</div>
              </div>
            </div>
        ...
        <div style={{borderTop:"1px solid rgba(255,255,255,.07)",paddingTop:22,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
          <div style={{fontSize:12,color:"rgba(255,255,255,.35)",fontWeight:400}}>© 2025 BanyanVision Fashion. All rights reserved.</div>
...
function AppShell() {
  ...
  if(loading)return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"var(--ivory)",flexDirection:"column",gap:20}}>
      <img src="/bv.jpg" alt="BanyanVision" style={{width:72,height:72,objectFit:"cover",borderRadius:16,boxShadow:"0 8px 32px rgba(194,24,91,.25)",animation:"float 2s ease-in-out infinite"}}/>
