...
import { Check, Copy, Heart, Lock, RefreshCw, Share2, TrendingUp, Truck, Zap } from "lucide-react";
...
const ProductDetailPage = ({ productId, setPage, toast }) => {
  ...
  const shareWhatsApp=()=>{
    const msg=encodeURIComponent(`Check out "${data?.name}" on BanyanVision!\n${productUrl()}`);
    window.open(`https://wa.me/?text=${msg}`,"_blank");
  };

  const shareNative=()=>{
    if(navigator.share){
      navigator.share({title:data?.name,text:`Check out this product on BanyanVision!`,url:productUrl()}).catch(()=>{});
    }else{
      copyLink();
    }
  };
        ...
        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:isMobile?24:64,marginBottom:52}}>
          ...
          <div>
            ...
            <div style={{display:"flex",gap:8,marginBottom:20,alignItems:"center"}}>
              <span style={{fontSize:11,fontWeight:700,color:"var(--muted)",letterSpacing:.5,textTransform:"uppercase",marginRight:4}}>Share:</span>
              <button onClick={copyLink} title="Copy link" style={{display:"flex",alignItems:"center",gap:6,padding:"8px 14px",borderRadius:10,border:"1.5px solid var(--border2)",background:copied?"var(--roseL)":"#fff",color:copied?"var(--rose)":"var(--text2)",fontSize:12,fontWeight:600,cursor:"pointer",transition:"all .2s"}}>
                <Ic icon={Copy} size={13}/>{copied?"Copied!":"Copy Link"}
              </button>
              <button onClick={shareWhatsApp} title="Share on WhatsApp" style={{display:"flex",alignItems:"center",gap:6,padding:"8px 14px",borderRadius:10,border:"1.5px solid #25D366",background:"#F0FFF4",color:"#128C7E",fontSize:12,fontWeight:700,cursor:"pointer",transition:"all .2s"}}
              ...
              <button onClick={shareNative} title="Share" style={{display:"flex",alignItems:"center",gap:6,padding:"8px 14px",borderRadius:10,border:"1.5px solid var(--border2)",background:"#fff",color:"var(--text2)",fontSize:12,fontWeight:600,cursor:"pointer",transition:"all .2s"}}
                onMouseEnter={e=>{e.currentTarget.style.borderColor="var(--rose)";e.currentTarget.style.color="var(--rose)";}}
                onMouseLeave={e=>{e.currentTarget.style.borderColor="var(--border2)";e.currentTarget.style.color="var(--text2)";}}>
                <Ic icon={Share2} size={13}/>Share
              </button>
            </div>
